...
export async function startCommand(name: string, options: { open?: boolean; check?: boolean } = {}): Promise<void> {
  ...
  try {
    ...
    await execAsync(
      `cd "${simPath}" && docker compose -p vsp-${pkg.name} up -d`,
      { cwd: simPath }
    );
...
function displayAccessInfo(pkg: NonNullable<ReturnType<typeof getPackage>>): void {
  console.log('');
  logger.success('═══════════════════════════════════════════════════════════════');
  console.log('');
